import { API_OBJECTS } from "@/constants/app";
import { ERROR_MESSAGES, STRINGS } from "@/constants/messages";
import { AccountService } from "@/services";
import { RequestHandler } from "express";
import { constructResponse } from "../utilities/common";

export class UsersController {
  static getMe: RequestHandler = async (req: any, res: any) => {
    try {
      return constructResponse({
        res,
        message: STRINGS.Success,
        code: 200,
        data: { ...req.user, role: undefined, isAdmin: req.user.role.includes("admin") },
        apiObject: API_OBJECTS.Account,
      });
    } catch (error) {
      return constructResponse({
        res,
        message: ERROR_MESSAGES.InternalServerError,
        code: 500,
        data: error,
        apiObject: API_OBJECTS.Account,
      });
    }
  };

  static updateMe: RequestHandler = async (req: any, res: any) => {
    try {
      const updated = await AccountService.updateAccount(req.user.id, req.body);
      
      return constructResponse({
        res,
        message: STRINGS.Success,
        code: 200,
        data: { ...updated, passwordHash: undefined, role: undefined, isAdmin: updated.role.includes("admin") },
        apiObject: API_OBJECTS.Account,
      });
    } catch (error) {
      return constructResponse({
        res,
        message: ERROR_MESSAGES.InternalServerError,
        code: 500,
        data: error,
        apiObject: API_OBJECTS.Account,
      });
    }
  };
}